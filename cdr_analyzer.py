...
def get_cdr_path():
    """Get CDR file path from command line, or prompt (supports drag-and-drop paste)."""
...
def main():
    ...
    if not os.path.exists(cdr_path):
        print()
        print("[ERROR] CDR file not found.")
        print()
        print("Usage:")
        print("python cdr_analyzer.py <cdr_file.csv>")
        print()
        sys.exit(1)
    ...
    if df.empty:
        print("[ERROR] No valid records found in the CDR file.")
        sys.exit(1)
    ...
    txt_path = os.path.join(REPORT_DIR, f"cdr_report_{report_file_time}.txt")
    pdf_path = os.path.join(REPORT_DIR, f"cdr_report_{report_file_time}.pdf")
    xlsx_path = os.path.join(REPORT_DIR, f"cdr_report_{report_file_time}.xlsx")
